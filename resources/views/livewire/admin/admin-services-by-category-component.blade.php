<div class="px-5">
    ...
    <h1 class="text-4xl mb-5">Admin service list</h1>
    ...
    <table class="min-w-full table-auto">
        ...
        <tr class="bg-white border-4 border-gray-200 text-center">
            <td class="px-16 py-2">{{ $service->id }}</td>
            <td class="px-16 py-2"><img width="100px" src="{{ asset('images/services/thumbnails') }}/{{ $service->thumbnail }}" alt=""></td>
            <td class="px-16 py-2">{{ $service->name }}</td>
            <td class="px-16 py-2">{{ $service->price }}</td>
            <td class="px-16 py-2">
                @if ($service->status)
                Active
                @else
            ...
            <td class="px-16 py-2">{{ $service->category->name }}</td>
            ...
            <td class="px-16 py-2">
                  ...
                  <a href="#" 
                    ...
                    onclick="confirm('Are you sure want to delete?') || event.stopImmediatePropagation()" wire:click.prevent="deleteService({{ $service->id }})">
                    Delete
                  </a>
              </td>
