<div class="px-5">
    ...
    <h1 class="text-4xl mb-5">Content</h1>
    ...
    <table class="min-w-full table-auto">
      <thead class="justify-between">
        <tr class="bg-gray-800">
          ...
          <th class="px-16 py-2">
            <span class="text-gray-300">More</span>
          </th>
      ...
      <tbody class="bg-gray-200">
        ...
        <tr class="bg-white border-4 border-gray-200 text-center">
          <td class="px-16 py-2">{{ $body->id }}</td>
          <td class="px-16 py-2"><img width="100px" class="inline-block" src="{{ asset('images/body') }}/{{ $body->image }}" alt=""></td>
          <td class="px-16 py-2">{{ $body->name }}</td>
          <td class="px-16 py-2">{{ $body->title }}</td>
          <td class="px-16 py-2">{{ $body->content }}</td>
          <td class="px-16 py-2">{{ $body->more }}</td>

          <td class="px-16 py-2">
              <a href="{{ route('admin.edit_dashboard', ['body_id' => $body->id]) }}" 
              ...
              <a href="#" 
                ...
                onclick="confirm('Are you sure want to delete?') || event.stopImmediatePropagation()" wire:click.prevent="deleteBody({{ $body->id }})">
                Delete
              </a>
          </td>
        </tr>
    ...
    <div class="flex space-x-2 justify-left">
        <a
          href="{{ route('admin.add_dashboard') }}"
          type="button"
